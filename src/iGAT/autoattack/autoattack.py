...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=False,
                 ...
                 device='cuda', log_path=None, mean_norm=None, std_norm=None, n_itr=None):
        self.model = model
        self.norm = norm
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=100 if n_itr is None else n_itr, verbose=False,
            ...
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=100 if n_itr is None else n_itr, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device, mean_norm=mean_norm, std_norm=std_norm)
        
            from .square import SquareAttack
            self.square = SquareAttack(self.model, p_init=.8, n_queries=5000 if n_itr is None else n_itr, eps=self.epsilon, norm=self.norm,
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100 if n_itr is None else n_itr, verbose=False,
            ...
            self.fab = FABAttack_TF(self.model, n_restarts=5, n_iter=100 if n_itr is None else n_itr, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device)
        
            from .square import SquareAttack
            self.square = SquareAttack(self.model.predict, p_init=.8, n_queries=5000 if n_itr is None else n_itr, eps=self.epsilon, norm=self.norm,
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                if self.verbose: print(num_robust)
                if num_robust == 0:
                    break
        ...
        if return_labels:
            return x_adv, y_adv, robust_accuracy# max_nr
        else:
            return x_adv, robust_accuracy# max_nr
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            ...
            #self.apgd_targeted.n_target_classes = 9
            self.square.n_queries = 5000 if self.n_itr is None else self.n_itr
